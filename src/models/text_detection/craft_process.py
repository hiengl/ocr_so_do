...
class CraftDetector:
    ...
    def _load_model(self):
        """Load CRAFT model."""
        from src.models.text_detection.craft_model import load_craft
        return load_craft(self.config['weights_path'], device=self.device)
    ...
    def cvt2heatmap_img(self, img):
        img = (np.clip(img, 0, 1) * 255).astype(np.uint8)
        img = cv2.applyColorMap(img, cv2.COLORMAP_JET)
        return img

    def warp_coord(self, Minv, pt):
        out = np.matmul(Minv, (pt[0], pt[1], 1))
        return np.array([out[0] / out[2], out[1] / out[2]])

    def get_det_boxes_core(self, textmap, linkmap, text_threshold, link_threshold, low_text):
        # prepare data
    ...
    def get_poly_core(self, boxes, labels, mapper):
        # configs
        num_cp = 5
        max_len_ratio = 0.7
        expand_ratio = 1.45
        ...
        for k, box in enumerate(boxes):
            ...
            # make final polygon
            poly = []
            poly.append(self.warp_coord(Minv, (spp[0], spp[1])))
            for p in new_pp:
                poly.append(self.warp_coord(Minv, (p[0], p[1])))
            poly.append(self.warp_coord(Minv, (epp[0], epp[1])))
            poly.append(self.warp_coord(Minv, (epp[2], epp[3])))
            for p in reversed(new_pp):
                poly.append(self.warp_coord(Minv, (p[2], p[3])))
            poly.append(self.warp_coord(Minv, (spp[2], spp[3])))
    ...
    def get_det_boxes(self, textmap, linkmap, text_threshold, link_threshold, low_text, poly=False):
        boxes, labels, mapper = self.get_det_boxes_core(textmap, linkmap, text_threshold, link_threshold, low_text)

        if poly:
            polys = self.get_poly_core(boxes, labels, mapper, linkmap)
        else:
            polys = [None] * len(boxes)

        return boxes, polys

    def adjust_result_coordinates(self, polys, ratio_w, ratio_h, ratio_net=2):
        if len(polys) > 0:
    ...
    def detect_text(self, image):
        ...
        img_resized, target_ratio, _ = self.resize_aspect_ratio(
            image,
    ...
    def crop_image(self, img, coord_box):
        """Crop image using coordinates."""
        if img is None or len(coord_box) == 0:
            return Image.new("RGB", (32, 32), (255, 255, 255))

        coord_box = np.array(coord_box, dtype=np.int32)
        x, y, w, h = cv2.boundingRect(coord_box)
        ...
        adjusted_coords = coord_box - np.array([x, y])
