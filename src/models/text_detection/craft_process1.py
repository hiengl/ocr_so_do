...
def detect_text(image, config, device):
    """Run CRAFT inference on an image."""
    net = load_craft(config['weights_path'], device=device)
...
def crop_image(img, coord_box):
    ...
    # Apply mask to ROI
    cropped_img = cv2.bitwise_and(roi, roi, mask=mask)
